...
void calSUB(OPN* opn2, OPN* opn1){ //opn2 - opn1
    ...
    Node *now1;
    Node *now2;
    now2 = opn2->Operator; //시작포인트 설정 (소수부 맨 끝자리), 두 수의 위치를 바꿈
    now1 = opn1->Operator;
    int upcount = 0; //소수 -> 정수로 넘어갈때 넘김수 구현
    int nextcount = 0;
    for(i=0; i<frcP; i++) { //소수부 계산
        num2 = (int)(now2->val) - 48; //chr -> int 변
        ...
        if(i == intP-2) { //맨앞자리
            if(num3 < 0) { //전체 수가 음수임, 값 뒤집어 줘야
                ...
                if (opn2->intPart == 2 && opn2->frcPart == 2 && opn1->intPart == 2 && opn1->intPart == 2) {
					...
					now2->val = num3*(-1)+ 47; //맨 앞 숫자는 한자리만 빼준다
                	now2 = now2->next;
                	while (now2->val != '.') { //정수부 반전
                		now2->val = 106 - now2->val;
                		now2 = now2->next;
					}
					now2 = now2->next;
                	for (j=0; j<frcP; j++) { //소수부 반전
