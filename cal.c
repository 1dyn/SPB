...
typedef struct OPNode{
    ...
    struct OPN *prev;
    struct OPN *next;
    int negative; // 음수 양수 체크  
    int intPart; // 정수부분 자릿수
    int frcPart; // 소수부분 자릿수
}OPN;
// 연산자와 피연산자 포인팅용 노드
...
OP *in_to_postfix(OP *inputlist){
    Stack *sign = newStack(); // 기호를 저장할 스택인 sign 선언
    OP *list = newOP(); // 후위표기법으로 바꾼 값을 새롭게 저장하는 OP선언 
    char val; // 파일에서 읽어온 한글자를 저장하는 변수 선언
    ...
    return list; // list(후위표기법으로 바꾼 DLL)를 리턴해줌
}
// return = list
...
void calculate(OP* OP){
    ...
    while(now != NULL){
        ...
        if(val == '+' || val == '-'){
            ...
            if(val == '+'){
                //printf("%c\n", opn1->Operator->val);
                if (opn2->negative == 0 && opn1->negative == 0){ //양수 + 양수
					calADD(opn2,opn1);
					ans = opn1;
    			}
    			else if (opn2->negative == 0 && opn1->negative == 1) { //양수 + 음수
    				opn1->negative = 0;
    				calSUB(opn2,opn1);
    				ans = opn2;
				}
				else if (opn2->negative == 1 && opn1->negative == 0) { //음수 + 양수
					opn2->negative = 0;
					calSUB(opn1,opn2);
					ans = opn1;
				}
...
void calSUB(OPN* opn2, OPN* opn1){ //opn2 - opn1
    ...
    if (opn2->negative == 1 && opn1->negative == 0){//음수 - 음수
        now2 = opn1->Operator; //시작포인트 설정 (소수부 맨 끝자리), 두 수의 위치를 바꿈
        now1 = opn2->Operator;
    }
    else{
        now1 = opn1->Operator; //시작포인트 설정 (소수부 맨 끝자리), 원래대로
        now2 = opn2->Operator;
    }
        
    int upcount = 0; //소수 -> 정수로 넘어갈때 넘김수 구현
    int nextcount = 0;
    for(i=0; i<frcP; i++) { //소수부 계산
        num2 = (int)(now2->val) - 48; //chr -> int 변형
        num1 = (int)(now1->val) - 48;
        num3 = num2 - num1 + nextcount; //뺄셈 진행
        nextcount = 0; //초기화 

        if(num3 < 0){ //0보다 작음
            Node *horse = now2->prev; //앞자리 horse
            now2->val = (num3 + 10) + 48; //뺄셈 결과값
         
            if((char)horse->val == '.'){ //앞자리가 "."이면 (현재 자리가 소수 첫째자리 라면)
                upcount = -1; //앞자리로 -1을 넘겨줌
            }
           else { //첫째자리가 아님
                horse->val -= 1; //앞자리 -1
            }
            ...
            now2->val = num3 + 48; //그대로 대입
        }
        now2 = now2->prev; //다음자리로 이동
        now1 = now1->prev;

    }
    if(now2->val == '.'){ // 현재 "."에 index 되어있다면 정수부로 이동
        now2 = now2->prev;
    }
    if(now1->val == '.'){
        now1 = now1->prev;
        ...
        //printf("%c %c", now1->val, now2->val);
    for(i=0; i<intP; i++){ //정수부 계
        num2 = (int)(now2->val) - 48; //chr -> int 변형
        num1 = (int)(now1->val) - 48;
        num3 = num2 - num1 + nextcount;
        if(i == intP-2) { //맨앞자리
            if(num3 < 0) { //전체 수가 음수임, 값 뒤집어 줘야
                opn2->negative = 1; //음수로 바꿈
                now2->val = num3*(-1) -1 + 48; //맨 앞 숫자는 한자리만 빼준다
                now2 = now2->next;
                while (now2->val != '-') {
                	now2->val = now2->val;
                    now2 = now2->next;
...
void main(){
    DLL *list = newDLL(); // 문자열 (전체 문장) 입력을 위한 리스트
    FILE *input = fopen("input.txt", "r");
    char *str;
    int cnt[2] = {0, 0};
    int t = 1; //정수 자릿수, 소수 자릿수 체크용도
    int ngtCheck = 0; // 음수 처리 용도
    int firstngt = 0; // 맨 처음에 음수 연산자가 오는경우 처리 
    
    OP *operand = newOP(); // 끝자리수 저장하기위한 OP

    while (fscanf(input, "%c", &str) != EOF) {
        if ((char)str != '=') {
            if ((char)str == '.') { // str이 '.'일 때
                if(t!=1){
                   printf("소수 입력 오류. \n잘못 입력된 이후의 값은 무시됩니다.\n");
                   break;
               }
            t = 0;
               ...
               while(now->next != NULL){
                 ...
                 if(list->head != NULL){
                  ...
                  t = 1;
                  Node *now = list->head;
                while(now->next != NULL) { // now포인터를 list의 맨끝으로 이동
                    now = now->next;
                }
                if(ngtCheck){
                   OPappend(operand, now, cnt[0], cnt[1], 1);
